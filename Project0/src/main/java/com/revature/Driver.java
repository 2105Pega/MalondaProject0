...
public class Driver {
	...
	private static void CustomerDetailScreen() {
		...
		String leftAlignFormat = "| %-5s | %-5s | %-15s | %-15s | %-9s | %-10s | %-10s | %-20s | %-20s | %-6s |%n";

		System.out.format(
				"+-------+-------+-----------------+-----------------+-------------+------------+--------------+----------------------+----------------------+--------+%n");
		System.out.format(
				"| ID    | Title | FirstName       | LastName        |    SSN      |    DOB     |  Phone       |  Email Address       | Adress               | ZipCode|%n");
		System.out.format(
				"+-------+-------+-----------------+-----------------+-------------+------------+--------------+----------------------+----------------------+--------+%n");
		for (Customer cust : customers.getCustomers()) {
			System.out.format(leftAlignFormat, cust.getCustomerNo(), cust.getTitle(), cust.getFirstName(),
					cust.getLastName(), cust.getSSN(), cust.getDOB(), cust.getPhone(), cust.getEmailAddress(),
	...
	private static void ManagerScreen() {
		...
		do {
			...
			System.out.println("	");
			System.out.println(
					"                          PLEASE ENTER [D] DEPOSIT, [w] TO WITHDRAW, [C] TO CHECK BALANCE, ");
			System.out.println(
					"                      [T] TO TRANSFER ,[A] VIEW ACCOUNTS, [I] TO VIEW CUSTOMERS INFORMATIONS,");
			System.out
					.println("                           [Q] APPROVE/DENY APPLICATION, [V] TO VIEW APPLICATION STATUS");
			System.out.print("                                                 OR [ X ] TO EXIT :  ");
